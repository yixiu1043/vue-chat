...
export const getPlanMsgStr = (plan, planName) => {
  ...
  plan.forEach((item) => {
    ...
    str += `${beginIssue}-${endIssue}期:${planName}${plans} ${status}\n`;
  });
  return str;
};
...
export const getRunningDomain = () => {
  if (process.env.NODE_ENV === 'development') {
    return '/api';
    // return 'http://wxchatdev2.mt204.com';
    // return 'http://localhost:6869';
  }
  const { protocol, hostname } = window.location;
  let domain = '';
  domain = `wxchatdev2.${hostname.split('.')[1]}.${hostname.split('.')[2]}`;
...
export const formatDate = (date) => {
  const mm = parseInt(date / 60, 10);
  const ss = parseInt(date % 60, 10);
  const m = mm >= 10 ? mm : `0${mm}`;
  const s = ss >= 10 ? ss : `0${ss}`;
  return `${m}:${s}`;
};

export const popupWindow = (url, type, w, h) => {

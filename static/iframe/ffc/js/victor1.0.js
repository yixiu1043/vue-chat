...
function sscAnimateEnd (o) {
  // console.log(o);
  animate.sscAnimateEnd(o.preDrawCode, '#numBig', o.counttime, o.lockCountDownTime, !1, o.nextLotteryNumber);
  // animate.sscAnimate("#numBig");
  videoTools.inseartData(o);//渲染小图
  datestr = o.counttime;
...
// 时时彩动画结束,大图中奖号码显示
animate.sscAnimateEnd = function (o, e, marketCountDownTime, lockCountDownTime, s, nextLotteryNumber) {
  clearInterval(animateID[e]);
  $('.oping').hide();// 开奖隐藏
  $('.djs').show();// 倒计时显示
  videoTools.cutTime(marketCountDownTime, lockCountDownTime);//开始倒计时
  const a = [];
  if (o.length <= 0) {
    // let flag = getData('upLotteryData');
    // if (flag && flag.type === 'ffc' && cutTime(flag.time) + 1 >= cutTime(getData('animationData').periodicalTime)) {
    //   o = flag.upLottery.split(',');
    // } else {
    // }
    o = upda(nextLotteryNumber).split(',');
  }
...
videoTools.xz3D = function (o, e) {
  ...
  e ? ($(o).children('.flip').eq(0).addClass('out')
    .removeClass('in'), setTimeout(function () {
...
videoTools.cutTime = function (marketCountDownTime, lockCountDownTime) {
  ...
  timerInterval = window.setInterval(function () {
    n -= 1;
    sum = lockCountDownTime + n;
    if (n >= 0) {
      updatContentTime(n);
    } else {
      // 开始跑封盘倒计时
      lockCountDownTimeContentUpdata(sum);
    }
  }, 1e3),
    animateID.timer = timerInterval;
};
// 封盘倒计时
function lockCountDownTimeContentUpdata (sum) {
  updatContentTime(sum);
  $('.lock').css('display', 'block');
  const a = [];
  showbluebg();
  const arr = returnCharacter([]);
  $('#numBig').find('.box').each(function () {
    $(this).find('span').removeClass();
    a.push($(this).find('span'));
  });
  videoTools.showNUM(a, []);
...
//显示蓝色背景；
function showbluebg () {
  $('.perspectiveView').each(function () {
    $(this).find('span').removeClass();
    $(this).find('span').eq(1).addClass('afterbg');
    $(this).find('span').eq(2).addClass('bigbg');
  });
}
// 返回内容

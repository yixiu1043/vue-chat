...
function disposeDate (data) {
  ...
  // s=中奖号码、
  let o = data.marketCountDownTime;
  let s = null;
  let flag = getData('upLotteryData');
  //获取存在本地的日期，和最新的日期进行对比，如果，存放的日期加1大于等于最新日期，就显示。为什么要加一，日期只能隔一个期数。隔太多的不显示；
  if (flag && flag.type == 'bjpk10' && cutTime(flag.time) + 1 >= cutTime(getData('animationData').periodicalTime)) {
    s = flag.upLottery;
  } else {
  ...
  $('#nextdrawid').text(data.nextPeriodicalTime);
  if (update(data.lotteryNumber).length > 0) {
    showcurrentresult(s);
    // 有奖
    ...
    showcurrentresult(update(data.nextLotteryNumber));
    // 无奖时动画
    $('#stat1_1').text('请');// 冠亚和
...
function formatAnimate (marketCountDownTime, lockCountDownTime) {
  ...
  countdowninv = setInterval(function () {
    ...
    countdown(sum, marketCountDownTime, lockCountDownTime);
...
function resultpage (e) {
  ...
  // timer = setTimeout(() => {
  //   let data = {
  //     marketCountDownTime: 2,//售卖时间倒计时，单位s
  //     lockCountDownTime: 1,//封盘时间倒计时，单位s
  //     lotteryNumber: "1,3,4,5,2,8,6,7,9,10",//中奖号码
  //     periodicalTime: '2019050310',//奖期
  //     periodicalNumber: '1234556',//期号
  //     size: getSize("10,9,3,4,5,6,7,8,2,1"),//大小
  //     longHu: longHus("10,9,3,4,5,6,7,8,2,1"),//龙虎
  //     carMoveTime: 5000,
  //   }
  //   disposeDate(data);
  //   clearTimeout(timer);
...
let countdowninv2;
// 车子加速动画
const animation3 = setInterval(function () {
  windflameani();
}, 150);
// 监听，页面加载后，隐藏地址栏

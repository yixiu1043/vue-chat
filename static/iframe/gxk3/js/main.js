$(() => {
  $('.animate').find('.loading').fadeOut(1000, () => { });
  // 点击开始模拟开奖;
 ...
 * t:中奖号码
*/
k3v.stopGame = function (t, data) {
  this.stop = function (t) {
    setTimeout(function () {
      clearInterval(animateId[t]);
      const n = $('#code').find('li');
      $(n).eq(t).attr('class', `k3v${0}`);
    }, 800 * t);
  };
  for (let e = 0; e < 3; e++) this.stop(e, t[e]);
  let timer = null;
  if (t.length <= 0) {
    //在第一次的情况下，是没有本地历史中奖号码，和没有当前中奖号，此时，就显示‘请稍等’；
    showContent(['请', '稍', '候'], t);
    if (data && data.marketCountDownTime) {
      //在倒计时时间走完后就把文字清除掉
      let timeOut = data.marketCountDownTime + data.lockCountDownTime;
      clearTimeout(timer);
      timer = setTimeout(() => {
        showContent(['', '', ''], t);
        clearTimeout(timer);
...
function noBetTime (marketCountDownTime, lockCountDownTime) {
  ...
  timer = setInterval(function () {
    ...
    if (num <= lockCountDownTime) {
      changeTime(num);
      $('#timetitle').text('封盘中...').css('color', 'yellow').css('font-size', '50px');
      k3v.stopGame([]);//封盘中显示请稍后
    }
    if (num <= 0) {
      // console.log(num);
      // $(".kaimodule").css('display', 'none')
...
k3v.dataEntrance = function (t) {
  ...
  if (flag && flag.type == 'gxk3' && cutTimeNumber(flag.time) + 1 >= cutTimeNumber(getData('animationData').periodicalTime)) {
    k3v.stopGame(flag.upLottery.split(','), t);// 传入中奖号码
  } else {
    if (t.preDrawCode.length <= 0) {
      k3v.stopGame(t.nextLotteryNumber.split(','), t);// 传入上一次号码
    } else {
      k3v.stopGame(t.preDrawCode, t);// 传入中奖号码
    }
  }
  k3v.updateData(t);
  setTimeout(function () {
